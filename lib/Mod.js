    ...
    if (node && node.type) { // var foo = require('./some/module')
        if (node.type == "VariableDeclarator" && 
          ...
          node.init.arguments[0].type == 'Literal') {
            ...
            ret.type = 'simple'; // need to call it something... TODO better names?
            ret.node = node;
            ret.name = node.id.name;
            ret.path = (/\.js$/.test(arg) ? arg : arg + '.js')/*,
          ...
          node.right.type == 'CallExpression' && node.right.callee.name == 'require' && 
