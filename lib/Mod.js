...
Mod.prototype.loadSrc = function (path, name) {
    ...
    try {
        ...
        console.log('Exception caught when trying to load ' + path + ' from ' + this.name + '.\n');
...
Mod.prototype.loadRequired = function () {
    ...
    each(getRequires(this.ast), function (r) {
        ...
        // this._requires[name].mod = this._mods[path][name];
        this._mods[path][name]._requiredBy.push(this.name);
    }, this);
    return this;
};
...
//      Identifier or to a MemberExpression
    ...
    while (--i) {
        for (var j = 0, k = levels[i].length; j < k; j++) {
            ...
            console.log(levels[i][j].name + ' requires ' + (levels[i][j]._requires.toString() || 'nothing'))
            if (!seen[current.name]) {
//                console.log('JUST SAW:', current.name);
//                console.log('REQUIRED BY', current._mods[current.path][current.name]._requiredBy);
