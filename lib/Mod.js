...
Mod.prototype.loadRequired = function () {
    ...
    each(getRequires(this.ast), function (r) {
        ...
        // can we/should we start fiddling with what goes where already?
        } else {
            if (r.path && r.path.length > 0 && r.path[1][0] && r.path[1][0].type == "VariableDeclaration") {
                console.log(r.path[1][0].indexOf(r.require.node));
                r.path[1][0].removeChild(r.require.node);
                console.log('removing', name, r.path[1][0].indexOf(r.require.node));
                hadRemoval = true;
            }
            if (!r.path[1][0].children.length) {
...
//    hadRemoval && console.log(JSON.stringify(this.ast))
    return this;
};
...
//        console.log();
    visit(ast, function (node, parent, path) {
        ...
        if (name == 'Widget' && node.type && node.type == 'VariableDeclarator')
            node.init && node.init.callee && node.init.callee.name && console.log(node.id.name)
        // node.type && node.type == 'VariableDeclarator' && node.id.name == 'canDefineXcessorsOnJSObject' && 
        ...
        if (require && seen[require.name]) {
            ...
            if (path && path.length > 0 && path[1][1] && path[1][1].type == "VariableDeclaration") {
                path[1][1].removeChild(require.node);
                if (!path[1][1].children.length)
                    console.log(path[1][1])

            }
            // TODO we need to remove empty expressions inside of functions, too
    ...
    name == 'Widget' && console.log(JSON.stringify(ast.first), getRequires(ast))
}
...
// removeRequire is called 3 times per module. This needs to be consolidated
    ...
    while (--i) {
        for (var j = 0, k = levels[i].length; j < k; j++) {
            ...
            console.log('\n\nthis is `' + current.name + '`, it is depended on by:');
            each(current._requiredBy, function (obj, i) {
                console.log(' ☞\t' + obj.name + ' at module scope ' + obj.atLevel)
            })

            console.log('this depends on:\n    ' + (Object.keys(current._requires).map(function(key) {
                return ' ' + key + (seen[key] ? '\tloaded' : '\tnot loaded')
            }).join('\n    ') || 'nothing'));

            if (current.name == 'Widget') {
                console.log('DEALING WITH WIDGET');
                fs.writeFileSync('widget-ast-before.json', JSON.stringify(current.ast), 'utf8');
            }
            ...
            fs.writeFileSync('widget-ast-after.json', JSON.stringify(current.ast), 'utf8');
            program.append(moduleClose(current.name));
        }
    }

    // now remove any requires statements left in the index/starting file
    index = levels[0][0];
    removeRequire(index.ast, seen);

    // iff it has any vars left
    indexVars = index.ast.first;
    indexVars && indexVars.type == 'VariableDeclaration' && vars.append(index.ast.children.shift());

    if (index.ast.first) // if there is anything left, copy it to the new program
        for (i = 0, l = index.ast.children.length; i < l; i++)
            program.append(index.ast.children.shift());
