 ...
 * @param  {Function(node, parent, ancestors)} [prefunc]  Function to call before
...
function visit (node, prefunc, postfunc, context, ancestors, parent, tokens) {
    var end;
    !tokens && (tokens = []);
    prefunc && (end = prefunc.call(context, node, parent, ancestors) || false);
    // if end is truthy, we can stop early
    if (end)
        return end;
    tokens = [node.type, tokens];
    var path = ancestors && Array.isArray(ancestors) ? [node, ancestors] : undefined;
    each(node, function (child, key) {
        if (child && typeof child == 'object') {
            visit(child, prefunc, postfunc, context, path, node, tokens);
        }
    });
//console.log(tokens);
    if (postfunc) {
