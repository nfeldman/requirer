    ...
    if (node && node.type) {
        if (node.type == 'VariableDeclarator' && node.init) {
            ...
            if (node.expression.type == 'AssignmentExpression' && 
              node.expression.right && node.expression.right.type == 'CallExpression' && 
              node.expression.right.callee.name == 'require' && node.expression.left) {
                ret.node = node;
                ret.id = node.expression.right.arguments[0].value;
                            // var foo; foo = require('./some/module') <-- maybe just avoid this form?
            } else if (node.expression.type == 'CallExpression' && node.expression.callee.name == 'require') {
                ret.node = node;
                ret.id = node.expression.arguments[0].value;
            }
        }
    }
